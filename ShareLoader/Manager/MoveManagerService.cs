...
namespace ShareLoader.Manager
{
    public class MoveManagerService : BackgroundService
    {
        ...
        public async Task MoveOther(List<DownloadItem> _items, DownloadGroup _group)
        {
            ...
            string pathTo = Path.Combine(_settings.MoveFolder, _group.Target.ToString(), "Other");
        ...
        public async Task MoveMovie(List<DownloadItem> _items, DownloadGroup _group)
        {
            ...
            string pathTo = Path.Combine(_settings.MoveFolder, _group.Target.ToString(), "Filme");
        ...
        public async Task MoveSoap(List<DownloadItem> _items, DownloadGroup _group)
        {
            ...
            foreach(string file in files)
            {
                ...
                if(dyn)
                {
                    ...
                    pathTo = Path.Combine(_settings.MoveFolder, _group.Target.ToString(), "Serien", _group.Sort, "Staffel " + m.Groups[1].Value);
                } else
                {
                    pathTo = Path.Combine(_settings.MoveFolder, _group.Target.ToString(), "Serien", _group.Sort);
                }
